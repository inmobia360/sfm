...
const notifications = createNotificationStore(); const system = createAgentSystem({ notificationStore: notifications });
const configured = createAgentSystem({ config });
assert.equal(configured.registry.get('INFANTE').divisionId, 'CORP');
assert.equal(configured.registry.get('JANITORIAL').role, 'CEO');
...
assert.equal(notifications.unreadCount('INFANTE'), 1);
