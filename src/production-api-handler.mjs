...
export function createProductionApiHandler({ state = {}, audit = [], idempotency = new Set() } = {}) {
  return function handle({ method = 'GET', path, context, action, resource, intent, decision, idempotencyKey } = {}) {
    ...
    const event = { auditEventId: `AUD-${audit.length + 1}`, actorId: resolved.actorId, divisionId: resolved.divisionId, action, status: result.status, requestId: resolved.requestId, traceId: resolved.traceId };
    audit.push(event);
